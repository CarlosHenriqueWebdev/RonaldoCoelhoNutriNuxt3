<template>
    <div class="lg:hidden">
        <div v-if="items.length > 0">
            <div :style="{ backgroundImage: `url(${`/assets/images/${items[0].backgroundImage}`})` }" class="py-[80px] bg-right">
    ...
    <div class="hidden lg:block">
        <swiper :loop="true" :autoplay="autoplay" :pagination="pagination">
            <swiper-slide v-for="(item, index) in items" :key="index">
                <div :style="{ backgroundImage: `url(${`/assets/images/${item.backgroundImage}`})` }" class="py-[132px] bg-right h-full select-none cursor-grab active:cursor-grabbing">
