<template>
    ...
    >
        ...
        >
            <TitleSection
                :subtitle="videosText.subtitle"
                :title="videosText.title"
            />
        </div>
        <ul
            class="flex flex-col gap-[64px] sm:grid sm:grid-cols-[1fr_1fr] lg:grid-cols-[1fr_1fr_1fr] sm:gap-x-[32px] sm:gap-y-[64px] w-full"
        >
            <li
                v-for="(item, index) in listItems"
                :key="index"
                class="w-full flex flex-col gap-[16px] h-fit"
            >
                ...
                <div class="flex flex-col gap-[8px] uppercase font-bold">
                    ...
                    <span class="text-[14px] text-vibrant-green">{{
                        item.lengthAndDate
                    }}</span>
                </div>
            </li>
...
export default {
    ...
    data() {
        return {
            videosText: content.videos,
            listItems: content.videos.videosList,
        };
    },
};
</script>
